...
export default function CheckoutPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 md:px-6 py-8 md:py-12 pt-24 md:pt-32">
      <h2 className="font-heading text-2xl md:text-3xl font-light text-espresso mb-6 md:mb-10">Đặt Hàng</h2>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
        {/* Shipping form */}
        <div className="order-2 lg:order-1">
          <h3 className="text-xs uppercase tracking-widest text-rose mb-6 md:mb-8">Thông Tin Giao Hàng</h3>
          <OrderForm
            cart={cart}
            total={tamTinh}
        ...
        <div className="order-1 lg:order-2">
          <h3 className="text-xs uppercase tracking-widest text-rose mb-6 md:mb-8">Tóm Tắt Đơn Hàng</h3>
          <div className="bg-blush/20 p-4 md:p-6">
            {/* Cart items */}
            <div className="space-y-4 md:space-y-5 mb-6">
              {cart.map((item) => (
                <div key={`${item.id}_${item.sizeChon ?? ""}`} className="flex items-start gap-3 md:gap-4">
                  <div className="relative w-12 md:w-14 flex-shrink-0 overflow-hidden bg-blush" style={{ aspectRatio: "3/4" }}>
                    {item.anhURL ? (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-stone mt-0.5">Size: <span className="font-medium text-espresso">{item.sizeChon || "Mặc định"}</span></p>
              ...
              {coupon ? (
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="p-2 text-stone text-xs hover:text-rose transition-colors"
                    aria-label="Xóa mã"
                  >
                    ✕
                  </button>
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  <input
                    ...
                    placeholder="Nhập mã..."
                    className="flex-1 bg-transparent border-b border-stone-300 py-2 px-0 text-espresso text-xs focus:outline-none focus:border-espresso transition-colors uppercase"
                  />
                  <button
